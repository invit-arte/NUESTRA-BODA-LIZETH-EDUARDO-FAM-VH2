...
function initCountdown() {
  ...
  const eventDate = new Date("October 10, 2026 15:00:00").getTime();
...
function initOpeningScreen() {
  ...
  /* Oculta los datos del invitado un segundo antes de terminar */
  sealVideo.addEventListener("timeupdate", function () {
    if (
      !guestWelcome ||
      !sealVideo.duration ||
      guestTextHidden
    ...
    const remainingTime =
      sealVideo.duration - sealVideo.currentTime;

    if (remainingTime <= 1) {
      guestTextHidden = true;
      guestWelcome.classList.add("is-hiding");
    }
  });
